...
!function(a, b) {
    ...
    var p = e.utils = {extend: function(a, c, d) {
            ...
            return c >= d ? a.pageX - b.pageX > 0 ? g : i : a.pageY - b.pageY > 0 ? h : f
    ...
    e.Instance = function(a, b) {
        ...
        return c(), this.element = a, this.enabled = !0, this.options = p.extend(p.extend({}, e.defaults), b || {}), this.options.stop_browser_behavior && p.toggleDefaultBehavior(this.element, this.options.stop_browser_behavior, !1), this.eventStartHandler = t.onTouch(a, m, function(a) {
            ...
            var a, b;
            for (this.options.stop_browser_behavior && p.toggleDefaultBehavior(this.element, this.options.stop_browser_behavior, !0), a = - 1; b = this.eventHandlers[++a]; )
    ...
    var q = null, r = !1, s = !1, t = e.event = {bindDom: function(a, b, c) {
            ...
            var d = this, f = function(f) {
                ...
                if (!g.match(/mouse/) || !s) {
                    g.match(/touch/) || g.match(/pointerdown/) || g.match(/mouse/) && 1 === f.which ? r = !0 : g.match(/mouse/) && !f.which && (r = !1), g.match(/touch|pointer/) && (s = !0);
                    var h = 0;
                    r && (e.HAS_POINTEREVENTS && b != o ? h = u.updatePointer(b, f) : g.match(/touch/) ? h = f.touches.length : s || (h = g.match(/up/) ? 0 : 1), h > 0 && b == o ? b = n : h || (b = o), (h || null === q) && (q = f), c.call(v, d.collectEventData(a, b, d.getTouchList(q, b), f)), e.HAS_POINTEREVENTS && b == o && (h = u.updatePointer(b, f))), h || (q = null, r = !1, s = !1, u.reset())
                }
            };
            ...
            var e = k;
            return(d.type.match(/mouse/) || u.matchType(j, d)) && (e = j), {center: p.getCenter(c), timeStamp: (new Date).getTime(), target: d.target, touches: c, eventType: b, pointerType: e, srcEvent: d, preventDefault: function() {
            ...
            if (this.current && !this.stopped) {
                ...
                return p.each(this.gestures, function(c) {
            ...
            var d, f, g = a.timeStamp - c.timeStamp, h = a.center.pageX - c.center.pageX, i = a.center.pageY - c.center.pageY, j = b.lastVelocityEvent, k = b.velocity;
            return j && a.timeStamp - j.timeStamp > e.UPDATE_VELOCITY_INTERVAL ? (k = p.getVelocity(a.timeStamp - j.timeStamp, a.center.pageX - j.center.pageX, a.center.pageY - j.center.pageY), b.lastVelocityEvent = a, b.velocity = k) : b.velocity || (k = p.getVelocity(g, h, i), b.lastVelocityEvent = a, b.velocity = k), a.eventType == o ? (d = b.lastEvent && b.lastEvent.interimAngle, f = b.lastEvent && b.lastEvent.interimDirection) : (d = b.lastEvent && p.getAngle(b.lastEvent.center, a.center), f = b.lastEvent && p.getDirection(b.lastEvent.center, a.center)), p.extend(a, {deltaTime: g, deltaX: h, deltaY: i, velocityX: k.x, velocityY: k.y, distance: p.getDistance(c.center, a.center), angle: p.getAngle(c.center, a.center), interimAngle: d, direction: p.getDirection(c.center, a.center), interimDirection: f, scale: p.getScale(c.touches, a.touches), rotation: p.getRotation(c.touches, a.touches), startEvent: c}), a
        }, register: function(a) {
            var c = a.defaults || {};
            ...
            if (!(b.options.drag_max_touches > 0 && a.touches.length > b.options.drag_max_touches))
                switch (a.eventType) {
                    case m:
                        ...
                        break;
                    case n:
                        if (a.distance < b.options.drag_min_distance && v.current.name != this.name)
                            return;
                        if (v.current.name != this.name && (v.current.name = this.name, b.options.correct_for_drag_min_distance && a.distance > 0)) {
                            var c = Math.abs(b.options.drag_min_distance / a.distance);
                            v.current.startEvent.center.pageX += a.deltaX * c, v.current.startEvent.center.pageY += a.deltaY * c, a = v.extendEventData(a)
                        }
                        (v.current.lastEvent.drag_locked_to_axis || b.options.drag_lock_to_axis && b.options.drag_lock_min_distance <= a.distance) && (a.drag_locked_to_axis = !0);
                        var d = v.current.lastEvent.direction;
                        a.drag_locked_to_axis && d !== a.direction && (a.direction = p.isVertical(d) ? a.deltaY < 0 ? h : f : a.deltaX < 0 ? g : i), this.triggered || (b.trigger(this.name + "start", a), this.triggered = !0), b.trigger(this.name, a), b.trigger(this.name + a.direction, a);
                        var e = p.isVertical(a.direction);
                        (b.options.drag_block_vertical && e || b.options.drag_block_horizontal && !e) && a.preventDefault();
                        break;
                    case o:
                        this.triggered && b.trigger(this.name + "end", a), this.triggered = !1
                    }
        }}, e.gestures.Hold = {name: "hold", index: 10, defaults: {hold_timeout: 500, hold_threshold: 1}, timer: null, handler: function(a, b) {
            switch (a.eventType) {
                case m:
